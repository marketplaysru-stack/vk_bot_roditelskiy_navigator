...
MAX_POSTS_PER_DAY = 4   # максимум постов из VK Feeds в сутки
...
def get_vk_api_token():
    return os.getenv("VK_TOKEN_PARENT")

def count_today_posts_from_vk(schedule):
    """Считает, сколько постов из VK Feeds уже запланировано на сегодня."""
    today_str = datetime.now().strftime("%Y-%m-%d")
    count = 0
    for item in schedule:
        if item.get("time", "").startswith(today_str) and item.get("source") == "vk_feed":
            count += 1
    return count
...
def fetch_and_generate_topics_from_vk(schedule, limit=3):
    """
    Проверяет группы, делает рерайт и возвращает список тем,
    но не больше MAX_POSTS_PER_DAY в сутки.
    """
    ...
    logger.info(f"✅ Найдено {len(new_topics)} новых тем из групп ВК (осталось лимитов на сегодня: {remaining})")
    return new_topics